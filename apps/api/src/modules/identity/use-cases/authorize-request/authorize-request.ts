import { evaluateAccess, permissionScope, type AccessTarget } from '../../domain/access.ts';
import type {
  AccessReader,
  ActiveMembership,
  SourcedGrant,
...
/**
 * Whether a route's declared target matches the scope its permission names: `:company` needs no target,
 * `:business` a business parameter, `:branch` a branch parameter; a `:platform` permission is never a route's.
 *
 * @param permission the 'action:resource:scope' permission
 * @param target     which target parameters the route declares
 * @param target.business whether a business parameter is declared
 * @param target.branch   whether a branch parameter is declared
 * @returns true when the declaration is consistent
 */
export function targetFitsPermission(
  permission: string,
  target: { business: boolean; branch: boolean },
): boolean {
  switch (permissionScope(permission)) {
    case 'company':
      return !target.business && !target.branch;
    case 'business':
      return target.business && !target.branch;
    case 'branch':
      return target.branch && !target.business;
    default:
      return false;
  }
}
