 ...
 * فمفيش event لتغيير متسجلش ولا تغيير من غير event. الـ company_id بييجي من app_company_id() مش من الـ caller،
...
export async function appendOutboxEvent(tx: Tx, id: string, event: OutboxEvent): Promise<void> {
  await tx.execute(sql`
    ...
    VALUES (app_company_id(), ${assertUuid(id, 'id')}, ${event.aggregateType},
            ${assertUuid(event.aggregateId, 'aggregateId')}, ${event.eventType},
