...
4. **Generate tasks.md**: Use TASKS_TEMPLATE_CONTENT (from the JSON output above) as the structure. For compatibility with older setup scripts that omit TASKS_TEMPLATE_CONTENT, read TASKS_TEMPLATE instead. Fill with:
   ...
   - Each phase includes: story goal, independent test criteria, tests (mandatory — domain unit, use-case integration, RLS negative, query shape + `EXPLAIN`), then implementation tasks
...
1. **From User Stories (spec.md)** - PRIMARY ORGANIZATION:
   ...
   - Map all related components to their story:
     ...
     - Tests specific to that story (always — `CLAUDE.md` §9)
   - Mark story dependencies (most stories should be independent)

2. **From Contracts**:
   - Map each interface contract → to the user story it serves
   - Each interface contract → contract test task [P] before implementation in that story's phase (always)
...
- **Phase 3+**: User Stories in priority order (P1, P2, P3...)
  - Within each story: Tests (always first, failing) → Models → Services → Endpoints → Integration
