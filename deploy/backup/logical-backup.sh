...
# Retention groups snapshots by host, and every `docker run --rm` gets a new hostname: a fixed one is what lets
# `forget` see last night's snapshot as the same series.
...
restic backup --tag logical --host "$BACKUP_HOST" --stdin-filename "$PGDATABASE.dump" --stdin-from-command -- \
  pg_dump --format=custom
restic forget --tag logical --host "$BACKUP_HOST" --prune \
